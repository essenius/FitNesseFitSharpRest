...
namespace Rest.Utilities
{
    internal static class FitNesseFormatter
    {
        ...
        /// <param name="utcNow">current date and time in UTC</param>
        /// <remarks>
        ///     the HttpCookie parser does not recognize the Max-Age attribute, so if it's there, we morph it into an Expires
        ///     attribute
        ...
        private static string RemoveInvalidCookieOptions(string cookieText)
        {
            ...
            if (fields.Length > 1)
                ...
                {
                    var trimmedField = fields[index].Trim();
                    if (string.IsNullOrEmpty(trimmedField))
                        continue;

                    var kvp = trimmedField.Split('=');
                    if (kvp.Length > 0 && CookieOptions.Contains(kvp[0].ToLower()))
                        stringBuilder.Append($";{trimmedField}");
                }
